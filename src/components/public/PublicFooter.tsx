...
export default function PublicFooter() {
  const { siteName, logoType, logoImageUrl, footer, newsletter } = useSiteContent();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="rounded-2xl border border-border bg-card p-6 sm:p-8 mb-10">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
            <div>
              <h3 className="text-2xl font-semibold text-text-primary">{newsletter.title}</h3>
              <p className="text-sm text-text-secondary mt-1">{newsletter.subtitle}</p>
            </div>
            ...
            <Link href="/" className="flex items-center gap-2.5 mb-4">
              <div className="w-9 h-9 bg-primary rounded-lg flex items-center justify-center overflow-hidden">
                {logoType === 'image' && logoImageUrl ? (
                  ...
                  <img src={logoImageUrl} alt={siteName} className="w-full h-full object-cover" />
                ) : (
              ...
              <span className="text-lg font-semibold text-text-primary tracking-tight">{siteName}</span>
            </Link>
            <p className="text-sm text-text-secondary leading-relaxed max-w-sm">{footer.description}</p>
          </div>

          {/* Dynamic columns */}
          {footer.columns.map(col => (
            <div key={col.title}>
              <h4 className="text-sm font-semibold text-text-primary mb-4 uppercase tracking-wider">{col.title}</h4>
