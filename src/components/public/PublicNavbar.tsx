...
function SiteLogo({ siteName, logoType, logoImageUrl }: { siteName: string; logoType: string; logoImageUrl: string }) {
  return (
    <Link href="/" className="flex items-center gap-2.5 shrink-0" aria-label={`${siteName} home`}>
      <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-primary overflow-hidden">
...
export default function PublicNavbar() {
  ...
  const { siteName, logoType, logoImageUrl, navLinks } = useSiteContent();
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 navbar-shell">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <SiteLogo siteName={siteName} logoType={logoType} logoImageUrl={logoImageUrl} />
